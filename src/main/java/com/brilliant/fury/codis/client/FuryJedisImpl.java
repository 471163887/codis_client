 ...
 *
 * 捕获的异常在JedisPoolManager中统一打印, CodisClient捕获的异常可以忽略
 *
 * @author by fury.
 * version 2020/4/1.
 */
@Slf4j
public class FuryJedisImpl implements FuryJedis {

    private String poolName;

    private JedisPoolManager poolManager = JedisPoolManager.getInstance();

    public FuryJedisImpl() {
        String firstPoolName = poolManager.getFirstPoolName();
        if (Strings.isNullOrEmpty(firstPoolName)) {
            throw new JedisException("[JedisPool_未初始化]");
        }
        this.poolName = firstPoolName;
    }

    public FuryJedisImpl(String poolName) {
        Jedis jedis = null;
        try {
            jedis = poolManager.getResource(poolName);
        } catch (Throwable t) {
            throw new JedisException("[JedisPool_未初始化]poolName=" + poolName);
        } finally {
            if (null != jedis) {
                jedis.close();
            }
